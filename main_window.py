...
from PyQt5.QtWidgets import QApplication, QSystemTrayIcon, QAction, QShortcut
from qfluentwidgets import FluentWindow, setTheme, Theme, MSFluentTitleBar, qconfig, InfoBar, InfoBarPosition, SystemTrayMenu
...
class MainWindow(FluentWindow):
    def __init__(self, start_minimized=False):
        ...
        startMinimizedSetting = str(settings.value("startMinimized", "false")).lower() == "true"
        autoStartEnabled = str(settings.value("autoStart", "false")).lower() == "true"
        
        if start_minimized or (startMinimizedSetting and autoStartEnabled):
            self.hide()
        else:
            self.show()
