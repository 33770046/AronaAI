import datetime
import re
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QLabel, QHBoxLayout, QGridLayout, QApplication
...
from qfluentwidgets import CardWidget, ComboBox, SwitchButton, PrimaryPushButton, TransparentPushButton, ScrollArea
from qfluentwidgets import qconfig, Theme, InfoBar, InfoBarPosition

CURRENT_VERSION = "dev.0.8.0"
...
class AboutPage(QWidget):
    ...
    def _setupUi(self):
        ...
        title = QLabel("关于", self._scrollContent)
        ...
        self._licenseLabel = QLabel("本程序遵循 GPLv3 开源许可", self)
        self._licenseLabel.setObjectName("license-label")
        self._licenseLabel.setAlignment(Qt.AlignCenter)
        outer.addWidget(self._licenseLabel)
    ...
    def _createAppInfoCard(self):
        ...
        version_label = QLabel(f"版本: dev.0.8.0", info)
        version_label.setObjectName("version-label")
        build_label = f"内部版本: dev.20260606.9000"
    ...
    def _applyStyles(self):
        ...
        for label in self.findChildren(QLabel, "link-label"):
            label.setStyleSheet(f"font-size: 14px; color: {accent};")
        ...
        for label in self.findChildren(QLabel, "license-label"):
            label.setStyleSheet(f"font-size: 12px; color: {fg_secondary};")
            
        for label in self.findChildren(QLabel, "status-label"):
            label.setStyleSheet(f"font-size: 14px; color: {status_green};")
